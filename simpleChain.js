...
class Block {
  constructor(address, star) {
    ...
    (this.time = 0), (this.previousBlockHash = "");
  }
}
...
class Blockchain {
      ...
      getLevelDBData("height")
        ...
        .catch(() => {
          addLevelDBData("height", -1).then(() => {
            this.addBlock(
              new Block("First block in the chain - Genesis block", "no star")
            );
          });
        });
    } catch (err) {
      ...
      this.getBlockHeight().then(height => {
        ...
        if (newBlock.height > 0) {
          this.getBlock(height).then(block => {
            ...
            newBlock.body.star.story = newBlock.body.star.story
              .split("")
              .map((l, i) => l.charCodeAt(0).toString(16))
              .join("");
  ...
  async find(cb) {
    return findLevelDBData(cb);
  }

  // Get block height
  getBlockHeight() {
    // return this.chain.length - 1;
    return getLevelDBData("height").catch(err =>
